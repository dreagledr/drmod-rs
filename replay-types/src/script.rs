...
/// Верхняя страховка длительности скрипта в кадрах (~4.6 ч при 60 FPS) —
/// `t + duration` команды не может её превысить.
///
/// Практический ограничитель — **размер тела запроса** (`MAX_BODY_BYTES` в
/// `src/api.rs`), а не эта константа: при 64 КиБ сжатого gzip сюда влезает
/// порядка 18 000 команд. Константа нужна против абсурдных значений: `t` и
/// `duration` — `u32`, и битый JSON с `t: 4_000_000_000` без неё переполнил бы
/// `t + duration`.
