...
/// здесь — только его проверка на текущем кадре.
...
/// поэтому нужно два `confirm`. Тип `RestartSpec` с дефолтами общий
/// (`drmod_replay_types::script`), а DIK-коды клавиш меню принадлежат моду —
/// поэтому сборка команд живёт здесь, а не в общем крейте.
fn restart_commands(spec: RestartSpec) -> Vec<ScriptCommand> {
    ...
    let mut t = 3 + spec.open_gap;
    for _ in 0..spec.ups {
        cmds.push(menu(addresses::DIK_UP, t));
        t += spec.hold + spec.gap;
    }
    for _ in 0..spec.downs {
        cmds.push(menu(addresses::DIK_DOWN, t));
        t += spec.hold + spec.gap;
    }
    for i in 0..spec.confirms {
        cmds.push(confirm(t));
        let last = i + 1 == spec.confirms;
        t += 3 + if last { spec.tail } else { spec.confirm_gap };
    }
    cmds
}
...
///
/// Включение: снять отрисовку (overlay + заглушки отрисовки геометрии игры)
/// **и** снять кап кадров — без снятого капа прогон упирается в пацер игры, а
/// без заглушек остаётся CPU-часть отрисовки. Прежний кап запоминается и
/// возвращается при выходе (ручные `/fps` не перетираем).
///
/// `Present` в этом режиме **не** снимается: замеры 2026-09-15 показали, что
/// выигрыша он не даёт (17.4–17.9 с на прогон против 13.8–15.1 с у заглушек
/// отрисовки), а вместе с заглушками роняет игру (`0xC0000005` в `d3d9.dll`).
/// Остаётся отдельным экспериментальным флагом `skip_present`.
///
...
///
/// * `{"headless": true}` — **рабочий режим прогона**: снять overlay и
///   заглушки отрисовки геометрии игры **и** кап кадров (`/fps` → `off`); по
///   концу прогона скрипта мод сам вернёт рендер и прежний кап
///   (`headless_service`). Без снятого капа прогон упирается в пацер игры, без
///   заглушек остаётся CPU-часть отрисовки. `Present` в этом режиме не
///   снимается: выигрыша он не даёт, а вместе с заглушками роняет игру
///   (`docs/HEADLESS.md` §5);
...
///     ⚠️ Экспериментальный: сам по себе безопасен, но **вместе со
///     `skip_draw` роняет игру** (AV в `d3d9.dll`), а выигрыша не даёт;
...
fn handle_script_run(body: &str, state: &Arc<Mutex<SharedState>>) -> (u16, Response) {
    ...
    let (phase_commands, phase_status, pending) = match req.restart {
        Some(spec) => {
            ...
            (restart_commands(spec), ScriptStatus::Restarting, Some(pending))
        }
        None => (
            req.commands,
