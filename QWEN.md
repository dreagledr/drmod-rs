...
- **TAS Editor `tas-editor/`** (самостоятельный крейт: свой `[workspace]` и `target/`, в корневой workspace не входит; лежит на уровне репо, а не в `tools/`): десктопный редактор TAS на **WinUI 3 через `windows-reactor`** — декларативные компоненты (`Component::create/update/view` + типизированные сообщения, без XAML) и **self-contained** развёртывание (`windows-reactor-setup` в `[build-dependencies]`, `build.rs` = `as_self_contained()`: приватный Windows App Runtime стейджится рядом с exe, манифест с маркером `windows-reactor-self-contained` встраивает линкер). Версии на 2026-09-20 — `windows-reactor` / `windows-reactor-setup` **0.100** (published 2026-09-03, edition 2024). Свой `.cargo/config.toml` фиксирует `x86_64-pc-windows-msvc`: корневой конфиг drmod-rs форсит `i686`, а под WinUI 3 / Windows App SDK он не собирается. Сборка и запуск: `cd tas-editor && cargo run --release`. ⚠️ Грабли self-contained (проверено 2026-09-20): `stage_pkg` считает NuGet-пакет готовым по факту существования `.nupkg` — ни код ответа `curl`, ни размер не проверяются, поэтому обрезанная загрузка (13 890 байт вместо 164 053 946) навсегда выглядит как «пакет есть»: сборка зелёная, а runtime не скопирован (симптом — рядом с exe лежит `Microsoft.Web.WebView2.Core.dll`, его копирование безусловное, но нет `Microsoft.WindowsAppRuntime.dll`). Диагностика: вывод build-скрипта (`Downloading`/`Extracting`/`MSIX not found at …`) виден только при `cargo build -v`, а `cargo clean -p tas-editor` build-скрипт **не** перезапускает (печатает `Fresh tas-editor`; сбрасывать надо `target/<triple>/<profile>/build/tas-editor-*`); лечение — удалить битый `.nupkg` и распакованный каталог рядом, положить корректный пакет с `api.nuget.org/v3-flatcontainer/...`, пересобрать. **Деплой — весь каталог профиля** (exe + staged DLL и локали, ~117 МБ), не только exe. Готовых агентских скиллов под Rust-реактор нет (у C#-версии `microsoft-ui-reactor` есть `SKILL.md` + `skills/` + CLI `mur`); ориентиры — `docs/crates/windows-reactor.md` и 55 сэмплов `crates/samples/reactor/*`. Статус 2026-09-20: заглушка hello world (`Component` + `window_title("TAS Editor")`), проверено запуском — окно `TAS Editor` открывается. Детали: `tas-editor/README.md`.
