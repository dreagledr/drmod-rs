...
- **TAS Editor (`tas-editor/`)**: десктопный TAS-редактор на WinUI 3 (`windows-reactor`, Rust, self-contained x64) — пока заглушка hello world
...
- **TAS Editor `tas-editor/`** (самостоятельный крейт: свой `[workspace]` и `target/`, в корневой workspace не входит; лежит на уровне репо, а не в `tools/`): десктопный редактор TAS на **WinUI 3 через `windows-reactor`** — декларативные компоненты (`Component::create/update/view` + типизированные сообщения, без XAML) и **self-contained** развёртывание (`windows-reactor-setup` в `[build-dependencies]`, `build.rs` = `as_self_contained()`: приватный Windows App Runtime стейджится рядом с exe, манифест с маркером `windows-reactor-self-contained` встраивает линкер). Версии на 2026-09-20 — `windows-reactor` / `windows-reactor-setup` **0.100** (published 2026-09-03, edition 2024). Свой `.cargo/config.toml` фиксирует `x86_64-pc-windows-msvc`: корневой конфиг drmod-rs форсит `i686`, а под WinUI 3 / Windows App SDK он не собирается. Сборка и запуск: `cd tas-editor && cargo run --release`. Поставка: `cd tas-editor && pwsh -File pack.ps1 -Build -Zip` — пересоздаёт `dist/` из каталога профиля (exe + ядро runtime; артефакты cargo `deps/` (~55 МБ), `build/`, `.fingerprint/`, `incremental/`, `*.pdb`, `*.d` выкидываются; локали прорежаются до `-KeepCultures` (`en-us`, `ru-RU`; `all` — оставить все); `Microsoft.Web.WebView2.Core.dll` — только с `-IncludeWebView2`), итог ≈56 МБ и zip ≈20 МБ вместо 117 МБ; скрипт падает, если ядро runtime неполное. ⚠️ Грабли self-contained (проверено 2026-09-20): `stage_pkg` считает NuGet-пакет готовым по факту существования `.nupkg` — ни код ответа `curl`, ни размер не проверяются, поэтому обрезанная загрузка (13 890 байт вместо 164 053 946) навсегда выглядит как «пакет есть»: сборка зелёная, а runtime не скопирован (симптом — рядом с exe лежит `Microsoft.Web.WebView2.Core.dll`, его копирование безусловное, но нет `Microsoft.WindowsAppRuntime.dll`). Диагностика: вывод build-скрипта (`Downloading`/`Extracting`/`MSIX not found at …`) виден только при `cargo build -v`, а `cargo clean -p tas-editor` build-скрипт **не** перезапускает (печатает `Fresh tas-editor`; сбрасывать надо `target/<triple>/<profile>/build/tas-editor-*`); лечение — удалить битый `.nupkg` и распакованный каталог рядом, положить корректный пакет с `api.nuget.org/v3-flatcontainer/...`, пересобрать. **Деплой — весь runtime-набор** (exe + staged DLL и подкаталоги), но не артефакты cargo: готовый дистрибутив собирает `pack.ps1` (~56 МБ, zip ~20 МБ), разбор состава — в `tas-editor/README.md`. Готовых агентских скиллов под Rust-реактор нет (у C#-версии `microsoft-ui-reactor` есть `SKILL.md` + `skills/` + CLI `mur`); ориентиры — `docs/crates/windows-reactor.md` и 55 сэмплов `crates/samples/reactor/*`. Статус 2026-09-20: **мок интерфейса** — `SplitView` со списком мок-скриптов, сворачиваемая полоса свойств (`name`, `trigger`, `restart`, поля выбранного кадра, условные команды `when_enemy`), таймлайн-матрица «кадр × действие» (строка — кадр, колонки — флаги действий + числовые оси камеры/стика; клик по галочке → RLE-пересборка команд) и JSON-редактор на `RichEditBox` с двусторонней связью (текст ↔ модель ↔ матрица). Данные — мок-скрипты в памяти (`src/mock.rs`), рабочая область на диске (`src/workspace.rs`: список/чтение/запись, настройки, `FolderPicker`) пока **не подключена** к сборке. ⚠️ Грабли рендера Reactor, найденные при отладке (2026-09-20): дети `Grid` без явного `grid_row`/`grid_column` схлопываются в (0,0); `content()`/`slot()`/`collection_slot()`/`tooltip()` возвращают `View` (завершают цепочку), для нескольких слотов — `slots([SlotView::new(..)])`; `Vec<View>` не реализует `IntoViews` (строки таблицы — `std::array::from_fn` → `[View; N]`); `ListView` не ограничивает высоту и распирает строку `Grid` — все строки `Auto` + явные `max_height`/`height` панелей (`Star`-строки отдают всю высоту); `TextBox` с `accepts_return(true)` остаётся однострочным → для многострочного JSON `RichEditBox`, у которого при чтении срезается хвост из ~1500 `\n`. Проверено: сборка debug/release, `cargo clippy --all-targets` (чисто), `cargo test` (17 тестов: model/matrix/workspace, расстановка колонок таймлайна, headless-монтирование раскладки), запуск `cargo run --release` со скриншотами окна. Headless-тесты: `windows_reactor::test` (`RecordingRuntime` + `Pump`, фича `test` в `[dev-dependencies]`) снаружи крейта **только монтирует** — применение команд живёт в цикле `App` (`Pump::dispatch_events` не публичен), поэтому после `mount_view`/`update_view` в записи остаётся `nodes=1, kinds=[]`, и структуру дерева проверить нельзя (сравнение видов доступно лишь по `Debug`-имени `MountedKind`): логику раскладки выносить в чистые функции (`panels::timeline::row_cells()` покрыт тестом «номер колонки на каждую ячейку»), а headless-тестом проверять только отсутствие ошибок планировщика. Высоты панелей считаются от `on_window_size` и задаются **самим контролам** (`ListView::max_height`, `RichEditBox::height`) — обёрткам они не подчиняются. Для скриншотов UI — `tas-editor/screenshot.ps1` (`PrintWindow` с `PW_RENDERFULLCONTENT`, иначе композиция WinUI даёт пустой кадр). Детали: `tas-editor/README.md`.
