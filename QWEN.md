...
- **Replay-types (`replay-types/`)**: Shared replay DTOs (`InputUnit`/`PlayerState`/`CameraState`, `#[repr(C)]`) + `to_bytes`/`from_bytes` — on-disk layout replay BLOB'ов; `input_bits` — биты действий в `InputUnit` (общие для мода и dbdump)
...
- **script_tuning** (`tools/script_tuning/`, python): перебор таймингов core-скрипта записи 117 (разгон → прыжок → хэви-атака у земли → риппер) для надёжного перелёта барьера 20 м в P310_RESTART. `core117.py` собирает очищенный скрипт (из 360 команд записи остаётся 7: выброшены 175 однокадровых команд камеры fi 131-558 и шумовые backward/left; высоту даёт root motion анимации тяжёлой атаки, лаунч на fi 96 — за 8 кадров до риппера), разгон всегда 5 кадров. `sweep.py` кладёт сетку `t_jump × t_attack` в `out\tuning\*.json` и с `--run` гоняет её через HTTP API, считая `max_y`/`cleared`/`spawn_ok` в `sweep.csv`. **Порядок прогона: взвод ДО рестарта** — триггер скрипта это проверка уровня (`segment::in_zone` каждый тик), поэтому скрипт взводится, пока игрок вне зоны спавна и прошлый прогон отыгран, и только потом жмётся рестарт миссии: на спавне игрок входит в зону, и скрипт стартует кадром 0 от начала миссии (взвод на спавне даёт старт ближайшим тиком — тайминги уезжают). `sweep.py` сам читает `GET /state`, не даёт взвестись в зоне и снимает скрипт при гонке. `selftest.py` — оффлайн-проверки (допуски `in_zone`, метрики, порядок взвода). Не сделан (нужна игра): автоматизация рестарта миссии через `pause`/`menu_*`/`confirm` — ввод в меню живьём не тестировался. Детали: `tools/script_tuning/README.md`.
