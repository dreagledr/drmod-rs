...
- **Меню Settings как пульт TAS-настроек** (`src/ui.rs` → `render_tas_controls`, обе сборки): прямо в окне настроек — фиксация шага времени движка (`POST /dt`: чекбокс «Фиксированный dt» + слайдер `dt, мс` + «Синтетические часы (m_fTicks)»), пин RNG решений ИИ (`POST /rng`: комбо `off/lo/mid/hi/seed/freeze` + поле сида) и кап кадров (`POST /fps`: комбо «как в игре/снят/свой лимит» + поле FPS). Контролы пишут тот же runtime-стейт, что и HTTP-ручки, через общие `api::set_fixed_dt`/`set_fixed_dt_ms`/`set_rng_pin`/`set_fps_cap` — источник истины один, значения не рассинхронизируются с API; `base_addr` для адресов `cSlowRateManager`/пацера берётся из `HelloHud`.
...
- **HTTP API** (`src/api.rs`, debug + release): own minimal HTTP server (raw `TcpListener`, no tiny_http) on `127.0.0.1:5223` — `POST /script/run` (JSON scripts in frames; optional `trigger: {pos: [x,y,z]}` arms the script — starts when the player enters the zone, like deferred record/playback (либо `trigger: {ticks: N}` — старт ровно через N тиков симуляции после взвода, в геймплее: фаза не плавает, прогоны воспроизводимы бит-в-бит); optional `restart: {...}` — мод сам рестартует миссию через меню паузы (`pause` → стрелка `dik_key` вверх → `confirm` ×2), ждёт loading и только затем взводится по `trigger`; статус фазы `restarting`), `GET /state`, `GET /logs?from_ms&to_ms&script_id`, `GET /health`, `POST /eject` (unloads the DLL: HTTP thread sets a flag, the render loop performs `shutdown()` + `hudhook::eject()`), `POST /order` (**заказ смены подфазы = рабочий скип катсцены:** `{"name": "P370_EVENT", "arg": 1, "clear_event": true}` — заказ штатной движковой функцией `0x94E1F0`; ⚠️ загрузку стопорит **пауза**: при выставленном `STA_PAUSE` (код 19 в `staFlags`, его ставит шаг 0 катсценного меню) заявка меняет только подпись подфазы (объект состояния `+0x38`) и запись чекпойнта, а сцена стоит на месте; **без паузы** движок реально выгружает текущую сцену и грузит заказанную (проверено 2026-09-12: монолог `P370_IN` → `P370_EVENT`, кадр сменился, загрузка идёт через найденное «пустое» состояние игрока); собственный пункт Skip катсценного меню на PC при этом инертен (индекс подтверждённого пункта никто не читает, меню виснет на шаге 5) — рабочий скип делает `cutscene_skip.py`: меню даёт консольный UX, а уборку меню и заказ подфазы выполняет инструмент; `clear_event` снимает флаг `STA_EVENT`, иначе внутри события движок заказ игнорирует; хэш имени = `CRC32(имя в нижнем регистре) & 0x7FFFFFFF`, phase id = hex цифр имени (`p370` = `0x370`); полный разбор механики и таблиц подфаз — `docs/PHASE.md`), `POST /phase` (движковый `changePhase` `0x8E3040`; на смену подфазы **не влияет** — задокументированный отрицательный результат), `POST /dt` (`{"fixed": true|false}` — фиксированный шаг времени движка, по умолчанию выключен; опции `ms` (величина шага) и `ticks` (синтетические часы, по умолчанию вкл). Детур `cSlowRateManager::updateFrameTime` (`0xA03970`, зовётся раз за итерацию главного цикла `0xA52510`) после оригинала пишет `m_fTickDifference` = ms, `m_fTickRate` = 1.0, `unit[0].m_fDelta` = 1.0 и ведёт **линейные часы** `m_fTicks` (+0x80) = база + N·ms, `base + 0x17E93B0`; состояние — в `/state` → `dt` (в т.ч. `frames`, `synth_ticks_ms`, `ticks`)), `POST /fps` (**кап кадров движка:** `{"cap":"off"|"game"}` или `{"fps": N}` — период пацера (`base + 0x1B206EC`, единицы 3·мс: 50 = 60 FPS, 100 = 30 FPS) перезаписывается каждый кадр из render-цикла; `0` → пацер не ждёт. Вместе с `POST /dt {"fixed":true}` снятый кап даёт ускорение прогонов ≈ FPS/60, потому что каждый кадр = ровно 1/60 с симуляции; разбор пацера и адреса — QWEN.md «Frame pacer», `docs/API.md` §3.13), `POST/GET /watch` (debug: аппаратная точка останова на **запись** — `DR0`/`DR7` на все потоки процесса, VEH ловит `STATUS_SINGLE_STEP` и пишет в `/watch` `Eip` писателя + адрес возврата вызывающего + кандидатов-возвратов со стека; `{"on":true,"enemy":true}` — на поле анимации ближайшего врага, `{"addr":"0x…"}` — на произвольный адрес, `{"on":false}` — снять)), `POST /rng` (`{"pin": "off|lo|mid|hi|seed|freeze"}` — пин/сид `randRange`/`randFloat` (глобальный LCG ИИ врага, состояние в `base + 0x19D0814`, функции `0x9DE2A0`/`0x9DE2D0`/`0x9DE300`): MinHook на все три; состояние LCG крутится оригиналом, подменяется только значение. `lo/mid/hi` — константа (детерминизм, но выбор ИИ фиксирован), `seed` — запись состояния на первом тике скрипта (исход зависит от порядка бросков между потоками), `freeze` — состояние **замораживается** (значение = детерминированная функция аргументов, порядок не важен; поведение «живое», сид управляет исходом — так подобран воспроизводимый прыжок врага); режим — в `/state` → `rng_pin`/`rng_seed`). Ring buffer of 3600 frames (60 s). Scripts stop automatically on loading and before eject. **Кадр скрипта = тик симуляции (2026-09-11):** ввод API-скрипта подаётся из детура `updateInputUnit` через очередь кадров (`api::feed_tick` + счётчик `SIM_TICKS`, по образцу `PLAYBACK_FEED` у replay) — render досчитывает кадры по числу прошедших тиков; в меню/загрузке (тика ввода нет) остаётся один кадр за кадр отрисовки, иначе встал бы скрипт меню/рестарта. До правки кадр скрипта шёл по кадрам отрисовки, и при 43–57 FPS один и тот же кадр (прыжок 45, удар 76) попадал в разные моменты физики: одна и та же связка давала 5/8 и 0/15. NumPad4 runs the builtin script through the same `ScriptRunner`. Design: `docs/API.md` (механика ввода меню — §10.5). The server is a single thread with non-blocking accept (10 ms stop-flag poll) and 1 s read/write timeouts per connection — `shutdown()` joins it in bounded time, so DLL eject never hangs (tiny_http was replaced because it had no socket timeouts and spawned unjoinable internal threads).
