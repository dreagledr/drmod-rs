...
- **TAS Editor C# (`tas-editor-cs/`)**: the same editor rebuilt on WinUI 3 via `Microsoft.UI.Reactor` — self-contained + NativeAOT; two-pane shell over a real on-disk workspace (a picked folder of `.tas` files: new / duplicate / delete, explicit save with Ctrl+S), a virtualized command table over mock frames, edited inline, a `.tas` text region with a live parse and the format's command reference in a pane beside it (the docking host's own splitter), and the script converter (API JSON ⇄ `.tas` text ⇄ table frames). ⚠️ **Own conventions, English-only UI and comments: `tas-editor-cs/QWEN.md`**
...
| TAS Editor (C#): local conventions, language, one-component-per-file, the on-disk workspace, script converter, PRI publish gotcha | `tas-editor-cs/QWEN.md`, `tas-editor-cs/README.md` |
