...
но `script_tick` его пока не применяет. Записи длиннее потолка
`MAX_SCRIPT_FRAMES` (1 000 000 кадров, ~4.6 ч) отклоняются — это верхняя
страховка, а не рабочий лимит: практический ограничитель `POST /script/run` —
размер тела запроса, а выгрузка идёт файлом.
