...
def main(argv=None):
    ...
    api.setup_stdout()
    h, mod = open_game()
    print(f"module base = 0x{mod:08X}; жду сцену (STA_EVENT + не пауза), "
    ...
    while time.time() - t0 < a.timeout:
        ...
        if flags is None or menu is None:
            time.sleep(0.5)
            continue
        if flags & STA_EVENT_MASK and menu != 3 and not fired:
            print(f"[{time.time() - t0:5.1f}s] сцена идёт (STA=0x{flags:08X}, "
                  f"menu={menu}) → заказ {a.name}", flush=True)
            try:
