...
- **Середина** — таймлайн-матрица: строка — кадр, колонки — все действия-флаги (`forward`, `jump`,
  `light_attack`, `blade`, …) и числовые оси камеры/стика. Клик по галочке переключает действие в кадре, после чего
  команды скрипта пересобираются RLE-нормализацией (`matrix::FrameMatrix::to_commands`). Список показывает ровно
  столько строк, сколько влезает в отведённую высоту (иначе `ListView` рисуется поверх нижних панелей) — остальные
  кадры листаются кнопками `▲`/`▼`.
- **Между таблицей и редактором** — тянущаяся полоса (свой `GridSplitter`: в реакторе его нет). Реагирует только
  на нажатие левой кнопки, с захватом указателя (`capture_pointer_on_press`), поэтому курсор можно уводить куда
  угодно. Доля высоты хранится в `split_ratio`, число видимых строк пересчитывается автоматически.
- **Снизу** — JSON-редактор (`RichEditBox`) в `STAR`-строке: занимает всё оставшееся место. Правка текста сразу
  пересобирает модель и матрицу; мок и текст связаны двусторонне.
...
- Горизонтальной прокрутки таймлайна нет: правые колонки (`wpn`, `codec`, `m-*`, числовые оси) уходят за край окна.
  `ScrollViewer` для этого не годится — ломает раскладку (контейнеры в реакторе не клипуют).
- Числа `cam_dx/cam_dy/stick_x/stick_y` в таблице только для чтения: их правка — в полосе свойств выбранного кадра
  или в JSON. `NumberBox` на каждую ячейку — это тысячи интерактивных контролов.
- `ListView` не подчиняется ни `height`, ни `max_height`, ни обёртке в `ScrollViewer` (растёт по строкам и рисуется
  поверх нижних панелей), поэтому число строк считается заранее: `Editor::visible_frames` (шаг строки `ROW_HEIGHT`,
  измерен по скриншоту — ~66 DIPs), а остальные кадры листаются кнопками `▲`/`▼`.
