...
| Назад | S | `0x800000` (бит 23) + `left_stick = [0, 1000]` |
| Влево | A | `0x200000` (бит 21) + `left_stick = [-1000, 0]` |
| Вправо | D | `0x100000` (бит 20) + `left_stick = [1000, 0]` |
| Прыжок | Space | `0x1` (бит 0) ⚠️ — открывает меню оружия (см. docs/API.md §10.1) |
| Лёгкая атака | ЛКМ (`mouse=1`) | `0x40` (бит 6) |
| Тяжёлая атака | ПКМ (`mouse=2`) | `0x80` (бит 7) |
| Ninja run | LCtrl | `0x4000` (бит 14, сопутствует FORWARD) |
