...
> **Документ-история** (рантайм-верификация ввода Record/Replay). Актуальный дизайн подачи ввода — [`REPLAY.md`](REPLAY.md); сводка граблей — [`PITFALLS.md`](PITFALLS.md).
...
| `+0x3184` | `m_bRipperModeEnabled` (1 = ripper включён) | i32 |
| `+0x40C8` | `m_nBladeModeType` (0 = BM выключен, 2 = BM включён) | i32 |
...
Вход в BM — это запись `m_nBladeModeType` (Pl0000+0x40C8) = `2` (заодно обнуляется `+0x4058`). Поймано аппаратной точкой останова на запись (`POST /watch {"on": true, "addr": "<player+0x40C8>"}`) при тапе BM с места:

| Что | RVA | Замечание |
|-----|-----|-----------|
| **активатор (вход)** | `0x7CD120` (`ret 4`) | запись `[entity+0x40C8] = 2` на `0x7CD18E`; entity резолвится по строке-классу (`0x1BE9EF4`) |
| **выход** | `0x78A123` | `[esi+0x40C8] = 0`; рядом тирдаун-группа `0x78A0B0`/`0x78A180` — снимает эффекты оружия и зовёт общий сеттер анимации `0x68CAF0` (`mov [ecx+0x618], eax`) |
| **`isBladeModeActive()`** | `0x795330` (vtable Pl0000 слот 203) | `1`, если `[+0x40C8] != 0`, **или** `r_anim == 0x45 (69)`, **или** `[0x1BEA090] & 0x20000000` (перед этим — ранний выход по подобъекту `+0x7D0`) |

- **Активатор не вызывается напрямую ниоткуда** (`callers_of.py` → 0 `call rel32` на `0x7CD120`): его адрес лежит в таблице хендлеров игрока в `.rdata` (слот `base + 0x12A1C6C`, в окружении — указатели на код `0x78xxxx`/`0x7Bxxxx`/`0x7Fxxxx`). То есть решение «пускать BM или нет» принимается **по состояниям** (какое состояние дёргает активатор), а не одним центральным флагом, — это и есть механика «BM доступен не во все фазы».
- Поле `+0x40C8` нигде больше не читается — только через `isBladeModeActive()`.
- Кнопка BM — `+0xE50` (`m_nButtonBlademode`), это **битмаска** (бит `0x800`): игра проверяет удержание как `mov reg, [esi+0xE50]; test [esi+0xCF8], reg` (то есть против `m_CurrentInput`); таких мест ~137 в `.text`.
- Успешный вход виден в логах мода: `r_anim` → `69`, `player.blade` → `2`.

**Проверка живьём** (связка перелёта барьера R-03, `P310_RESTART`, детерминированный мир `/dt fixed` + `/rng freeze seed 1`): бит `0x800`, поданный в лаунч-анимации (`r_anim 94`), игра **игнорирует** (`blade_mode_type` остаётся 0, траектория бит-в-бит как без тапа), а в фазе падения (`r_anim 11`) — **принимает** (тап с кадра 190 работает, с 188 — нет). На земле вход работает в беге (`r_anim 3`) и ninja (`71`), в атаках (`81` наземная, `94`) — нет. Свип кадра тапа — `tools/script_tuning/blade_tap_tune.py`.
