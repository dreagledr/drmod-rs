...
| `weapon_select` | ⚠️ | хук `isKeyDown(0x8D)` | 🔧 фикс: бит 0x01 = DPAD_LEFT (геймпад), не клавиша "2" — битовый путь листает слоты. Реализован хук `isKeyDown`/`isKeyPressed` (0x9D93A0/0x9D9400), подача через `set_raw_key(KEY_WEAPON_SELECT=0x8D)`. Требуется тестирование |
...
- **Работает (после фиксов):** движение 4 стороны, `left_stick`, `jump` (бит 0x10 + удержание),
  ...
  от subweapon), `ar_mode` (бит 0x08 + фронт), `camera` (yaw + pitch; значения как у реальной
  мыши, см. таблицу), `light_attack`/`heavy_attack` (ранее).
- **Не работает:** меню-клавиши и цифры 2/3 (raw-кэш), `lock_on`/
  `camera_reset` (цикл 5..22), `weapon_select`/`codec`/`pause`/
